...
async def identify_product(ocr_text: str) -> dict:
    ...
    prompt = f"""You are a product identification AI. Analyze the following OCR text extracted from a product label.
OCR Text: "{ocr_text}"

Rules:
- Ignore marketing claims, percentages, barcodes, weights, and generic words like "INGREDIENTS", "NATURAL", "TESTED", "MOISTURIZING", "48H".
- Focus ONLY on clear brand names, product names, and category indicators.
- If you cannot confidently identify the product, return null values. Do NOT guess or hallucinate.
